...
func TestE2EBridgeL1CrossDomainMessenger(t *testing.T) {
	...
	require.NoError(t, err)
	sentMsgReceipt, err := utils.WaitReceiptOK(testCtx, testSuite.L1Client, sentMsgTx.Hash())
	require.NoError(t, err)
	...
	// wait for processor catchup
	depositReceipt, err := utils.WaitReceiptOK(testCtx, testSuite.L2Client, transaction.L2TransactionHash)
	require.NoError(t, err)
	require.NoError(t, utils.WaitFor(testCtx, 500*time.Millisecond, func() (bool, error) {
...
func TestE2EBridgeL2CrossDomainMessenger(t *testing.T) {
	...
	require.NoError(t, err)
	sentMsgReceipt, err := utils.WaitReceiptOK(testCtx, testSuite.L2Client, sentMsgTx.Hash())
	require.NoError(t, err)
