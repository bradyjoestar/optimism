...
func TestE2EBridgeTransfersOptimismPortalETHReceive(t *testing.T) {
	...
	require.NoError(t, err)
	portalDepositReceipt, err := utils.WaitReceiptOK(testCtx, testSuite.L1Client, portalDepositTx.Hash())
	require.NoError(t, err)
...
func TestE2EBridgeTransfersL2ToL1MessagePasserReceive(t *testing.T) {
	...
	require.NoError(t, err)
	l2ToL1WithdrawReceipt, err := utils.WaitReceiptOK(testCtx, testSuite.L2Client, l2ToL1MessagePasserWithdrawTx.Hash())
	require.NoError(t, err)
